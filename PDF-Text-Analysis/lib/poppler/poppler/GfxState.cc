...
// GfxState.h
static const char *gfxColorSpaceModeNames[] = { "DeviceGray", "CalGray",
		"DeviceRGB", "CalRGB", "DeviceCMYK", "Lab", "ICCBased", "Indexed",
...
static const std::map<unsigned int, unsigned int>::size_type CMSCACHE_LIMIT =
		2048;
...
GfxColorTransform::GfxColorTransform(void *transformA, int cmsIntentA,
		unsigned int inputPixelTypeA, unsigned int transformPixelTypeA) {
...
GfxColorSpace* GfxColorSpace::parse(GfxResources *res, Object *csObj,
		OutputDev *out, GfxState *state, int recursion) {
	GfxColorSpace *cs;
	Object obj1;
	...
	if (csObj->isName()) {
		if (csObj->isName("DeviceGray") || csObj->isName("G")) {
			...
			error(errSyntaxWarning, -1, "Bad color space '{0:s}'",
					csObj->getName());
		}
		...
		if (obj1.isName("DeviceGray") || obj1.isName("G")) {
			...
			cs = GfxIndexedColorSpace::parse(res, csObj->getArray(), out, state,
					recursion);
		} else if (obj1.isName("Separation")) {
			cs = GfxSeparationColorSpace::parse(res, csObj->getArray(), out,
					state, recursion);
		} else if (obj1.isName("DeviceN")) {
			cs = GfxDeviceNColorSpace::parse(res, csObj->getArray(), out, state,
					recursion);
		} else if (obj1.isName("Pattern")) {
			cs = GfxPatternColorSpace::parse(res, csObj->getArray(), out, state,
					recursion);
		} else {
		...
		error(errSyntaxWarning, -1,
				"Bad color space - expected name or array or dict");
	}
	return cs;
}

void GfxColorSpace::createMapping(
...
void GfxDeviceGrayColorSpace::getGray(const GfxColor *color,
		GfxGray *gray) const {
	*gray = clip01(color->c[0]);
}

void GfxDeviceGrayColorSpace::getGrayLine(unsigned char *in, unsigned char *out,
		int length) {
...
void GfxDeviceGrayColorSpace::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
...
void GfxDeviceGrayColorSpace::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
	for (int i = 0; i < length; i++) {
		*out++ = in[i];
		*out++ = in[i];
...
void GfxDeviceGrayColorSpace::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
	for (int i = 0; i < length; i++) {
		*out++ = 0;
		*out++ = 0;
...
void GfxDeviceGrayColorSpace::getDeviceNLine(unsigned char *in,
...
void GfxDeviceGrayColorSpace::getCMYK(const GfxColor *color,
		GfxCMYK *cmyk) const {
	cmyk->c = cmyk->m = cmyk->y = 0;
	cmyk->k = clip01(gfxColorComp1 - color->c[0]);
}

void GfxDeviceGrayColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
...
static const double xyzrgb[3][3] = { { 3.240449, -1.537136, -0.498531 }, {
		-0.969265, 1.876011, 0.041556 }, { 0.055643, -0.204026, 1.057229 } };
...
GfxColorSpace* GfxCalGrayColorSpace::parse(Array *arr, GfxState *state) {
	...
	cs->kr = 1
			/ (xyzrgb[0][0] * cs->whiteX + xyzrgb[0][1] * cs->whiteY
					+ xyzrgb[0][2] * cs->whiteZ);
	cs->kg = 1
			/ (xyzrgb[1][0] * cs->whiteX + xyzrgb[1][1] * cs->whiteY
					+ xyzrgb[1][2] * cs->whiteZ);
	cs->kb = 1
			/ (xyzrgb[2][0] * cs->whiteX + xyzrgb[2][1] * cs->whiteY
					+ xyzrgb[2][2] * cs->whiteZ);
#ifdef USE_CMS
	cs->transform =
			(state != nullptr) ? state->getXYZ2DisplayTransform() : nullptr;
#endif
	return cs;
}
...
void GfxCalGrayColorSpace::getXYZ(const GfxColor *color, double *pX, double *pY,
		double *pZ) const {
...
void GfxCalGrayColorSpace::getGray(const GfxColor *color, GfxGray *gray) const {
	...
	*gray =
			clip01(
					(GfxColorComp) (0.299 * rgb.r + 0.587 * rgb.g
							+ 0.114 * rgb.b + 0.5));
}
...
void GfxCalGrayColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxCMYK cmyk;
	clearGfxColor(deviceN);
	getCMYK(color, &cmyk);
...
void GfxDeviceRGBColorSpace::getGray(const GfxColor *color,
		GfxGray *gray) const {
	*gray = clip01(
			(GfxColorComp) (0.3 * color->c[0] + 0.59 * color->c[1]
					+ 0.11 * color->c[2] + 0.5));
}

void GfxDeviceRGBColorSpace::getGrayLine(unsigned char *in, unsigned char *out,
		int length) {
	int i;

	for (i = 0; i < length; i++) {
		out[i] = (in[i * 3 + 0] * 19595 + in[i * 3 + 1] * 38469
				+ in[i * 3 + 2] * 7472) / 65536;
	}
}
...
void GfxDeviceRGBColorSpace::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
	for (int i = 0; i < length; i++) {
		*out++ = *in++;
		*out++ = *in++;
...
void GfxDeviceRGBColorSpace::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
	for (int i = 0; i < length; i++) {
		*out++ = *in++;
		*out++ = *in++;
...
void GfxDeviceRGBColorSpace::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
	GfxColorComp c, m, y, k;
...
void GfxDeviceRGBColorSpace::getDeviceNLine(unsigned char *in,
		unsigned char *out, int length) {
	GfxColorComp c, m, y, k;
...
void GfxDeviceRGBColorSpace::getCMYK(const GfxColor *color,
		GfxCMYK *cmyk) const {
	GfxColorComp c, m, y, k;
...
void GfxDeviceRGBColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxCMYK cmyk;
	clearGfxColor(deviceN);
	getCMYK(color, &cmyk);
...
void GfxCalRGBColorSpace::getXYZ(const GfxColor *color, double *pX, double *pY,
		double *pZ) const {
	double A, B, C;
...
void GfxCalRGBColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxCMYK cmyk;
	clearGfxColor(deviceN);
	getCMYK(color, &cmyk);
...
void GfxDeviceCMYKColorSpace::getGray(const GfxColor *color,
		GfxGray *gray) const {
	*gray = clip01(
			(GfxColorComp) (gfxColorComp1 - color->c[3] - 0.3 * color->c[0]
...
static inline void GfxDeviceCMYKColorSpacegetRGBLineHelper(unsigned char *&in,
...
void GfxDeviceCMYKColorSpace::getRGBLine(unsigned char *in, unsigned int *out,
		int length) {
	double r, g, b;
	for (int i = 0; i < length; i++) {
		GfxDeviceCMYKColorSpacegetRGBLineHelper(in, r, g, b);
		*out++ = (dblToByte(clip01(r)) << 16) | (dblToByte(clip01(g)) << 8)
				| dblToByte(clip01(b));
	}
}

void GfxDeviceCMYKColorSpace::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
	double r, g, b;
...
void GfxDeviceCMYKColorSpace::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
	double r, g, b;
...
void GfxDeviceCMYKColorSpace::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
	for (int i = 0; i < length; i++) {
		*out++ = *in++;
		*out++ = *in++;
...
void GfxDeviceCMYKColorSpace::getDeviceNLine(unsigned char *in,
		unsigned char *out, int length) {
	for (int i = 0; i < length; i++) {
		for (int j = 0; j < SPOT_NCOMPS + 4; j++)
			out[j] = 0;
...
void GfxDeviceCMYKColorSpace::getCMYK(const GfxColor *color,
		GfxCMYK *cmyk) const {
...
void GfxDeviceCMYKColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	clearGfxColor(deviceN);
...
// (not multiply by the white point)
void GfxLabColorSpace::getXYZ(const GfxColor *color, double *pX, double *pY,
		double *pZ) const {
	double X, Y, Z;
	double t1, t2;
...
void GfxLabColorSpace::getRGB(const GfxColor *color, GfxRGB *rgb) const {
	...
	if (transform != nullptr && transform->getTransformPixelType() == PT_RGB) {
		...
		return;
			...
			!= nullptr&& transform->getTransformPixelType() == PT_CMYK) {
		unsigned char out[gfxColorMaxComps];
		double in[gfxColorMaxComps];
		double c, m, y, k, c1, m1, y1, k1, r, g, b;
...
void GfxLabColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxCMYK cmyk;
	clearGfxColor(deviceN);
	getCMYK(color, &cmyk);
...
GfxColorSpace* GfxICCBasedColorSpace::parse(Array *arr, OutputDev *out,
	...
	const Ref iccProfileStreamA =
			obj1Ref.isRef() ? obj1Ref.getRef() : Ref::INVALID();
#ifdef USE_CMS
	// check cache
	if (out && iccProfileStreamA != Ref::INVALID()) {
		if (auto *item = out->getIccColorSpaceCache()->lookup(
				iccProfileStreamA)) {
	...
	if (obj2.isNull()
			|| !(altA = GfxColorSpace::parse(nullptr, &obj2, out, state,
					recursion + 1))) {
		switch (nCompsA) {
		case 1:
	...
	if (altA->getNComps() != nCompsA) {
		error(errSyntaxWarning, -1,
				"Bad ICCBased color space - N doesn't match alt color space");
		delete altA;
		return nullptr;
	}
	...
	if (obj2.isArray() && obj2.arrayGetLength() == 2 * nCompsA) {
		for (i = 0; i < nCompsA; ++i) {
			...
			cs->rangeMax[i] = obj2.arrayGet(2 * i + 1).getNumWithDefaultValue(
					1);
		}
	}
	...
	if (!hp) {
		...
		auto dhp =
				(state != nullptr && state->getDisplayProfile() != nullptr) ?
						state->getDisplayProfile() : nullptr;
		if (!dhp) {
		...
		COLORSPACE_SH(cst) | CHANNELS_SH(nCompsA) | BYTES_SH(1), dhp.get(),
		COLORSPACE_SH(dcst) | CHANNELS_SH(dNChannels) | BYTES_SH(1), cmsIntent,
		LCMS_FLAGS)) == nullptr) {
			error(errSyntaxWarning, -1, "Can't create transform");
			cs->transform = nullptr;
		} else {
			cs->transform = std::make_shared<GfxColorTransform>(transform,
					cmsIntent, cst, dcst);
		}
		if (dcst == PT_RGB || dcst == PT_CMYK) {
			...
			CHANNELS_SH(nCompsA) | BYTES_SH(1), dhp.get(),
				...
				cs->lineTransform = std::make_shared<GfxColorTransform>(
						transform, cmsIntent, cst, dcst);
			}
		}
	}
	// put this colorSpace into cache
	if (out && iccProfileStreamA != Ref::INVALID()) {
		out->getIccColorSpaceCache()->put(iccProfileStreamA,
				static_cast<GfxICCBasedColorSpace*>(cs->copy()));
	}
#endif
	return cs;
}

void GfxICCBasedColorSpace::getGray(const GfxColor *color,
		GfxGray *gray) const {
#ifdef USE_CMS
	if (transform != nullptr && transform->getTransformPixelType() == PT_GRAY) {
		...
		if (nComps == 3 && transform->getInputPixelType() == PT_Lab) {
			...
			in[1] = colToByte(
					dblToCol((colToDbl(color->c[1]) + 128.0) / 255.0));
			in[2] = colToByte(
					dblToCol((colToDbl(color->c[2]) + 128.0) / 255.0));
		} else {
		...
		if (nComps <= 4) {
			...
			std::map<unsigned int, unsigned int>::iterator it = cmsCache.find(
					key);
			if (it != cmsCache.end()) {
		...
		*gray =
				clip01(
						(GfxColorComp) (0.3 * rgb.r + 0.59 * rgb.g
								+ 0.11 * rgb.b + 0.5));
	}
#else
...
void GfxICCBasedColorSpace::getRGB(const GfxColor *color, GfxRGB *rgb) const {
	...
	if (transform != nullptr && transform->getTransformPixelType() == PT_RGB) {
		...
		if (nComps <= 4 && cmsCache.size() <= CMSCACHE_LIMIT) {
			...
			unsigned int value = (dblToByte(r) << 16) + (dblToByte(g) << 8)
					+ dblToByte(b);
			cmsCache.insert(std::pair<unsigned int, unsigned int>(key, value));
		}
	} else {
...
#ifdef USE_CMS
	if (lineTransform
			!= nullptr&& lineTransform->getTransformPixelType() == PT_RGB) {
		unsigned char *tmp = (unsigned char*) gmallocn(3 * length,
...
void GfxICCBasedColorSpace::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
#ifdef USE_CMS
	if (lineTransform
			!= nullptr&& lineTransform->getTransformPixelType() == PT_RGB) {
		...
		gfree(tmp);
			...
			!= nullptr&& lineTransform->getTransformPixelType() == PT_CMYK) {
		unsigned char *tmp = (unsigned char*) gmallocn(4 * length,
				sizeof(unsigned char));
		lineTransform->doTransform(in, tmp, length);
		unsigned char *current = tmp;
		double c, m, y, k, c1, m1, y1, k1, r, g, b;
...
void GfxICCBasedColorSpace::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
#ifdef USE_CMS
	if (lineTransform
			!= nullptr&& lineTransform->getTransformPixelType() == PT_RGB) {
...
void GfxICCBasedColorSpace::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
#ifdef USE_CMS
	if (lineTransform
			!= nullptr&& lineTransform->getTransformPixelType() == PT_CMYK) {
...
void GfxICCBasedColorSpace::getDeviceNLine(unsigned char *in,
		unsigned char *out, int length) {
#ifdef USE_CMS
	if (lineTransform
			!= nullptr&& lineTransform->getTransformPixelType() == PT_CMYK) {
...
void GfxICCBasedColorSpace::getCMYK(const GfxColor *color,
		GfxCMYK *cmyk) const {
#ifdef USE_CMS
	if (transform != nullptr && transform->getTransformPixelType() == PT_CMYK) {
		unsigned char in[gfxColorMaxComps];
		...
		if (nComps <= 4 && cmsCache.size() <= CMSCACHE_LIMIT) {
			...
			unsigned int value = (out[0] << 24) + (out[1] << 16) + (out[2] << 8)
					+ out[3];
			cmsCache.insert(std::pair<unsigned int, unsigned int>(key, value));
		}
	} else if (nComps
			!= 4&& transform != nullptr && transform->getTransformPixelType() == PT_RGB) {
		GfxRGB rgb;
		GfxColorComp c, m, y, k;
...
void GfxICCBasedColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxCMYK cmyk;
	clearGfxColor(deviceN);
	getCMYK(color, &cmyk);
...
void GfxICCBasedColorSpace::getDefaultRanges(double *decodeLow,
		double *decodeRange, int maxImgPixel) const {
...
GfxColorSpace* GfxIndexedColorSpace::parse(GfxResources *res, Array *arr,
		OutputDev *out, GfxState *state, int recursion) {
	GfxColorSpace *baseA;
	int indexHighA;
	Object obj1;
	...
	if (indexHighA < 0 || indexHighA > 255) {
		...
		if (indexHighA < 0)
			...
			indexHighA = 255;
		error(errSyntaxWarning, -1,
				"Bad Indexed color space (invalid indexHigh value, was {0:d} using {1:d} to try to recover)",
				previousValue, indexHighA);
	}
	...
	if (obj1.isStream()) {
		...
		for (i = 0; i <= indexHighA; ++i) {
			...
			for (j = readChars; j < n; ++j) {
				error(errSyntaxWarning, -1,
						"Bad Indexed color space (lookup table stream too short) padding with zeroes");
				cs->lookup[i * n + j] = 0;
			}
		}
		obj1.streamClose();
...
GfxColor* GfxIndexedColorSpace::mapColorToBase(const GfxColor *color,
	...
	if (likely(
			(idx + n - 1 < (indexHigh + 1) * base->getNComps()) && idx >= 0)) {
		p = &lookup[idx];
		for (i = 0; i < n; ++i) {
...
void GfxIndexedColorSpace::getRGBLine(unsigned char *in, unsigned int *out,
		int length) {
	unsigned char *line;
	int i, j, n;
...
void GfxIndexedColorSpace::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
	unsigned char *line;
	int i, j, n;
...
void GfxIndexedColorSpace::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
	unsigned char *line;
	int i, j, n;
...
void GfxIndexedColorSpace::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
	unsigned char *line;
	int i, j, n;
...
void GfxIndexedColorSpace::getDeviceNLine(unsigned char *in, unsigned char *out,
		int length) {
	unsigned char *line;
	int i, j, n;
...
void GfxIndexedColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	GfxColor color2;
...
GfxSeparationColorSpace::GfxSeparationColorSpace(GooString *nameA,
		GfxColorSpace *altA, Function *funcA) {
	name = nameA;
	alt = altA;
	func = funcA;
...
GfxSeparationColorSpace::GfxSeparationColorSpace(GooString *nameA,
		GfxColorSpace *altA, Function *funcA, bool nonMarkingA,
...
GfxColorSpace* GfxSeparationColorSpace::parse(GfxResources *res, Array *arr,
		OutputDev *out, GfxState *state, int recursion) {
...
void GfxSeparationColorSpace::getGray(const GfxColor *color,
		GfxGray *gray) const {
	double x;
...
void GfxSeparationColorSpace::getCMYK(const GfxColor *color,
		GfxCMYK *cmyk) const {
	double x;
	double c[gfxColorMaxComps];
	GfxColor color2;
...
void GfxSeparationColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	clearGfxColor(deviceN);
...
void GfxSeparationColorSpace::createMapping(
		std::vector<GfxSeparationColorSpace*> *separationList,
		int maxSepComps) {
	if (nonMarking)
		return;
	...
	default:
		...
		for (std::size_t i = 0; i < separationList->size(); i++) {
			...
			if (!sepCS->getName()->cmp(name)) {
				if (sepCS->getFunc()->hasDifferentResultSet(func)) {
					error(errSyntaxWarning, -1,
							"Different functions found for '{0:t}', convert immediately",
							name);
					gfree(mapping);
					mapping = nullptr;
					return;
		...
		if ((int) separationList->size() == maxSepComps) {
			error(errSyntaxWarning, -1,
					"Too many ({0:d}) spots, convert '{1:t}' immediately",
					maxSepComps, name);
			gfree(mapping);
			mapping = nullptr;
			return;
...
GfxDeviceNColorSpace::GfxDeviceNColorSpace(int nCompsA,
		std::vector<std::string> &&namesA, GfxColorSpace *altA, Function *funcA,
		std::vector<GfxSeparationColorSpace*> *sepsCSA) :
		nComps(nCompsA), names(std::move(namesA)) {
	alt = altA;
	func = funcA;
	sepsCS = sepsCSA;
	nonMarking = true;
...
GfxDeviceNColorSpace::GfxDeviceNColorSpace(int nCompsA,
		const std::vector<std::string> &namesA, GfxColorSpace *altA,
		Function *funcA, std::vector<GfxSeparationColorSpace*> *sepsCSA,
...
GfxColorSpace* GfxDeviceNColorSpace::parse(GfxResources *res, Array *arr,
		OutputDev *out, GfxState *state, int recursion) {
	int nCompsA;
	...
	if (arr->getLength() == 5) {
		...
		if (obj2.isDict()) {
			...
			for (int i = 0; i < colorants->getLength(); i++) {
				...
				if (obj3.isArray()) {
					GfxSeparationColorSpace *cs =
							(GfxSeparationColorSpace*) GfxSeparationColorSpace::parse(
									res, obj3.getArray(), out, state,
									recursion);
					if (cs) {
					...
					error(errSyntaxWarning, -1,
							"Bad DeviceN color space (colorant value entry is not an Array)");
					goto err5;
				}
			}
		}
...
void GfxDeviceNColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	clearGfxColor(deviceN);
	if (mapping == nullptr) {
		GfxCMYK cmyk;
...
void GfxDeviceNColorSpace::createMapping(
		std::vector<GfxSeparationColorSpace*> *separationList,
		int maxSepComps) {
	if (nonMarking)               // None
		return;
	...
	for (int i = 0; i < nComps; i++) {
		if (names[i] == "None") {
			...
			for (std::size_t j = 0; j < separationList->size(); j++) {
				...
				if (!sepCS->getName()->cmp(names[i])) {
					if (sepFunc != nullptr
							&& sepCS->getFunc()->hasDifferentResultSet(
									sepFunc)) {
						error(errSyntaxWarning, -1,
								"Different functions found for '{0:s}', convert immediately",
								names[i].c_str());
						gfree(mapping);
						mapping = nullptr;
						overprintMask = 0xffffffff;
			...
			if (!found) {
				if ((int) separationList->size() == maxSepComps) {
					error(errSyntaxWarning, -1,
							"Too many ({0:d}) spots, convert '{1:s}' immediately",
							maxSepComps, names[i].c_str());
					gfree(mapping);
					mapping = nullptr;
					overprintMask = 0xffffffff;
					return;
				...
				newOverprintMask |= startOverprintMask;
				if (nComps == 1)
					separationList->push_back(
							new GfxSeparationColorSpace(new GooString(names[i]),
									alt->copy(), func->copy()));
				else {
					for (const GfxSeparationColorSpace *sepCS : *sepsCS) {
						if (!sepCS->getName()->cmp(names[i])) {
							found = true;
							separationList->push_back(
									(GfxSeparationColorSpace*) sepCS->copy());
							break;
						}
					}
					if (!found) {
						error(errSyntaxWarning, -1,
								"DeviceN has no suitable colorant");
						gfree(mapping);
						mapping = nullptr;
						overprintMask = 0xffffffff;
						return;
...
GfxColorSpace* GfxPatternColorSpace::parse(GfxResources *res, Array *arr,
		OutputDev *out, GfxState *state, int recursion) {
	...
	if (arr->getLength() == 2) {
		obj1 = arr->get(1);
		if (!(underA = GfxColorSpace::parse(res, &obj1, out, state,
				recursion + 1))) {
			error(errSyntaxWarning, -1,
...
void GfxPatternColorSpace::getDeviceN(const GfxColor *color,
		GfxColor *deviceN) const {
	clearGfxColor(deviceN);
	deviceN->c[3] = 1;
}
...
GfxPattern::GfxPattern(int typeA, int patternRefNumA) :
		type(typeA), patternRefNum(patternRefNumA) {
...
GfxShadingPattern* GfxShadingPattern::parse(GfxResources *res, Object *patObj,
		OutputDev *out, GfxState *state, int patternRefNum) {
	Dict *dict;
	GfxShading *shadingA;
	double matrixA[6];
	Object obj1;
...
GfxShadingPattern::GfxShadingPattern(GfxShading *shadingA,
		const double *matrixA, int patternRefNumA) :
...
GfxShading* GfxShading::parse(GfxResources *res, Object *obj, OutputDev *out,
		GfxState *state) {
	GfxShading *shading;
	Dict *dict;
	int typeA;
	...
	if (!obj1.isInt()) {
		error(errSyntaxWarning, -1,
				"Invalid ShadingType in shading dictionary");
		return nullptr;
	}
	typeA = obj1.getInt();
	...
	case 4:
		if (obj->isStream()) {
			shading = GfxGouraudTriangleShading::parse(res, 4, dict,
	...
	case 5:
		if (obj->isStream()) {
			shading = GfxGouraudTriangleShading::parse(res, 5, dict,
					obj->getStream(), out, state);
		} else {
	...
	case 6:
		if (obj->isStream()) {
			shading = GfxPatchMeshShading::parse(res, 6, dict, obj->getStream(),
					out, state);
		} else {
	...
	case 7:
		if (obj->isStream()) {
			shading = GfxPatchMeshShading::parse(res, 7, dict, obj->getStream(),
					out, state);
		} else {
...
bool GfxShading::init(GfxResources *res, Dict *dict, OutputDev *out,
		GfxState *state) {
	Object obj1;
	int i;
	...
	if (obj1.isArray()) {
		if (obj1.arrayGetLength() == 4) {
			...
			if (!hasBBox) {
				error(errSyntaxWarning, -1,
						"Bad BBox in shading dictionary (Values not numbers)");
			}
		} else {
...
GfxFunctionShading::GfxFunctionShading(double x0A, double y0A, double x1A,
		double y1A, const double *matrixA, Function **funcsA, int nFuncsA) :
		GfxShading(1) {
	int i;
...
GfxFunctionShading* GfxFunctionShading::parse(GfxResources *res, Dict *dict,
	...
	if (obj1.isArray() && obj1.arrayGetLength() == 4) {
		...
		if (!decodeOk) {
			error(errSyntaxWarning, -1,
					"Invalid Domain array in function shading dictionary");
			return nullptr;
		}
	}
	...
	if (obj1.isArray() && obj1.arrayGetLength() == 6) {
		...
		if (!decodeOk) {
			error(errSyntaxWarning, -1,
					"Invalid Matrix array in function shading dictionary");
			return nullptr;
		}
	}
	...
	shading = new GfxFunctionShading(x0A, y0A, x1A, y1A, matrixA, funcsA,
			nFuncsA);
...
GfxUnivariateShading::GfxUnivariateShading(int typeA, double t0A, double t1A,
...
int GfxUnivariateShading::getColor(double t, GfxColor *color) {
	...
	if (cacheSize > 0) {
		...
		if (cacheBounds[lastMatch - 1] >= t) {
			upper = std::lower_bound(cacheBounds, cacheBounds + lastMatch - 1,
					t);
			lastMatch = upper - cacheBounds;
			lastMatch = std::min<int>(std::max<int>(1, lastMatch),
					cacheSize - 1);
		} else if (cacheBounds[lastMatch] < t) {
			upper = std::lower_bound(cacheBounds + lastMatch + 1,
					cacheBounds + cacheSize, t);
			lastMatch = upper - cacheBounds;
			lastMatch = std::min<int>(std::max<int>(1, lastMatch),
					cacheSize - 1);
		}
		...
		for (int i = 0; i < nFuncs; ++i) {
			if (funcs[i]->getInputSize() != 1) {
				error(errSyntaxWarning, -1,
						"Invalid shading function (input != 1)");
				break;
			}
...
void GfxUnivariateShading::setupCache(const Matrix *ctm, double xMin,
	...
	cacheBounds = (double*) gmallocn_checkoverflow(maxSize,
			sizeof(double) * (nComps + 2));
	...
	if (cacheSize != 0) {
		...
		for (j = 0; j < cacheSize; ++j) {
			...
			for (i = 0; i < nFuncs; ++i) {
				funcs[i]->transform(&cacheBounds[j],
						&cacheValues[j * nComps + i]);
			}
		}
	}
...
GfxAxialShading* GfxAxialShading::parse(GfxResources *res, Dict *dict,
		OutputDev *out, GfxState *state) {
...
GfxRadialShading::GfxRadialShading(double x0A, double y0A, double r0A,
		double x1A, double y1A, double r1A, double t0A, double t1A,
		Function **funcsA, int nFuncsA, bool extend0A, bool extend1A) :
...
GfxRadialShading* GfxRadialShading::parse(GfxResources *res, Dict *dict,
		OutputDev *out, GfxState *state) {
...
inline void radialEdge(double num, double den, double delta, double lower,
		double upper, double dr, double mindr, bool &valid, double *range) {
...
inline void radialCorner1(double x, double y, double &b, double dx, double dy,
		double cr, double dr, double mindr, bool &valid, double *range) {
	b = (x) * dx + (y) * dy + cr * dr;
...
inline void radialCorner2(double x, double y, double a, double &b, double &c,
		double &d, double dx, double dy, double cr, double inva, double dr,
		double mindr, bool &valid, double *range) {
	b = (x) * dx + (y) * dy + cr * dr;
	c = (x) * (x) + (y) * (y) - cr * cr;
	d = b * b - a * c;
	if (d >= 0) {
...
void GfxRadialShading::getParameterRange(double *lower, double *upper,
	...
	if (xMin >= xMax || yMin >= yMax
			|| (fabs(r0 - r1) < RADIAL_EPSILON
					&& (std::min<double>(r0, r1) < RADIAL_EPSILON
							|| std::max<double>(fabs(x0 - x1), fabs(y0 - y1))
									< 2 * RADIAL_EPSILON))) {
		*lower = *upper = 0;
		return;
	}
	...
	if (fabs(a) < RADIAL_EPSILON * RADIAL_EPSILON) {
		...
		// circles touching each corner
		radialCorner2(xMin, yMin, a, b, c, d, dx, dy, cr, inva, dr, mindr,
				valid, range);
		radialCorner2(xMin, yMax, a, b, c, d, dx, dy, cr, inva, dr, mindr,
				valid, range);
		radialCorner2(xMax, yMin, a, b, c, d, dx, dy, cr, inva, dr, mindr,
				valid, range);
		radialCorner2(xMax, yMax, a, b, c, d, dx, dy, cr, inva, dr, mindr,
				valid, range);
	}
...
GfxGouraudTriangleShading::GfxGouraudTriangleShading(int typeA,
		GfxGouraudVertex *verticesA, int nVerticesA, int (*trianglesA)[3],
		int nTrianglesA, Function **funcsA, int nFuncsA) :
		GfxShading(typeA) {
	int i;
...
GfxGouraudTriangleShading::GfxGouraudTriangleShading(
	...
	vertices = (GfxGouraudVertex*) gmallocn(nVertices,
...
GfxGouraudTriangleShading* GfxGouraudTriangleShading::parse(GfxResources *res,
		int typeA, Dict *dict, Stream *str, OutputDev *out,
		GfxState *gfxState) {
	...
	while (true) {
		...
		if (nVerticesA == vertSize) {
			...
			verticesA = (GfxGouraudVertex*) greallocn_checkoverflow(verticesA,
					vertSize, sizeof(GfxGouraudVertex));
		...
		for (i = 0; i < nComps; ++i) {
			verticesA[nVerticesA].color.c[i] = dblToCol(
					cMin[i] + cMul[i] * (double) c[i]);
		}
		++nVerticesA;
		bitBuf->flushBits();
		if (typeA == 4) {
			if (state == 0 || state == 1) {
				...
				if (nTrianglesA == triSize) {
					...
					trianglesA = (int (*)[3]) greallocn(trianglesA, triSize * 3,
							sizeof(int));
				}
				if (state == 2) {
	...
	shading = new GfxGouraudTriangleShading(typeA, verticesA, nVerticesA,
			trianglesA, nTrianglesA, funcsA, nFuncsA);
...
void GfxGouraudTriangleShading::getTriangle(int i, double *x0, double *y0,
		GfxColor *color0, double *x1, double *y1, GfxColor *color1, double *x2,
...
void GfxGouraudTriangleShading::getParameterizedColor(double t,
		GfxColor *color) const {
	double out[gfxColorMaxComps];
...
void GfxGouraudTriangleShading::getTriangle(int i, double *x0, double *y0,
		double *color0, double *x1, double *y1, double *color1, double *x2,
		double *y2, double *color2) {
	int v;
...
GfxPatchMeshShading::GfxPatchMeshShading(int typeA, GfxPatch *patchesA,
		int nPatchesA, Function **funcsA, int nFuncsA) :
		GfxShading(typeA) {
	int i;
...
GfxPatchMeshShading* GfxPatchMeshShading::parse(GfxResources *res, int typeA,
		Dict *dict, Stream *str, OutputDev *out, GfxState *state) {
	...
	while (true) {
		...
		if (nPatchesA == patchesSize) {
			...
			memset(patchesA + oldPatchesSize, 0,
					(patchesSize - oldPatchesSize) * sizeof(GfxPatch));
		}
		...
		if (typeA == 6) {
			...
			case 1:
				...
				for (j = 0; j < nComps; ++j) {
					p->color[0][0].c[j] =
							patchesA[nPatchesA - 1].color[0][1].c[j];
					p->color[0][1].c[j] =
							patchesA[nPatchesA - 1].color[1][1].c[j];
			...
			case 2:
				...
				for (j = 0; j < nComps; ++j) {
					p->color[0][0].c[j] =
							patchesA[nPatchesA - 1].color[1][1].c[j];
					p->color[0][1].c[j] =
							patchesA[nPatchesA - 1].color[1][0].c[j];
					p->color[1][1].c[j] = c[0][j];
					p->color[1][0].c[j] = c[1][j];
				}
				break;
			case 3:
				...
				for (j = 0; j < nComps; ++j) {
					p->color[0][0].c[j] =
							patchesA[nPatchesA - 1].color[1][0].c[j];
					p->color[0][1].c[j] =
							patchesA[nPatchesA - 1].color[0][0].c[j];
					p->color[1][1].c[j] = c[0][j];
					p->color[1][0].c[j] = c[1][j];
				}
				break;
	...
	if (typeA == 6) {
		for (i = 0; i < nPatchesA; ++i) {
			...
			p->x[1][1] = (-4 * p->x[0][0] + 6 * (p->x[0][1] + p->x[1][0])
					- 2 * (p->x[0][3] + p->x[3][0])
					+ 3 * (p->x[3][1] + p->x[1][3]) - p->x[3][3]) / 9;
			p->y[1][1] = (-4 * p->y[0][0] + 6 * (p->y[0][1] + p->y[1][0])
					- 2 * (p->y[0][3] + p->y[3][0])
					+ 3 * (p->y[3][1] + p->y[1][3]) - p->y[3][3]) / 9;
			p->x[1][2] = (-4 * p->x[0][3] + 6 * (p->x[0][2] + p->x[1][3])
					- 2 * (p->x[0][0] + p->x[3][3])
					+ 3 * (p->x[3][2] + p->x[1][0]) - p->x[3][0]) / 9;
			p->y[1][2] = (-4 * p->y[0][3] + 6 * (p->y[0][2] + p->y[1][3])
					- 2 * (p->y[0][0] + p->y[3][3])
					+ 3 * (p->y[3][2] + p->y[1][0]) - p->y[3][0]) / 9;
			p->x[2][1] = (-4 * p->x[3][0] + 6 * (p->x[3][1] + p->x[2][0])
					- 2 * (p->x[3][3] + p->x[0][0])
					+ 3 * (p->x[0][1] + p->x[2][3]) - p->x[0][3]) / 9;
			p->y[2][1] = (-4 * p->y[3][0] + 6 * (p->y[3][1] + p->y[2][0])
					- 2 * (p->y[3][3] + p->y[0][0])
					+ 3 * (p->y[0][1] + p->y[2][3]) - p->y[0][3]) / 9;
			p->x[2][2] = (-4 * p->x[3][3] + 6 * (p->x[3][2] + p->x[2][3])
					- 2 * (p->x[3][0] + p->x[0][3])
					+ 3 * (p->x[0][2] + p->x[2][0]) - p->x[0][0]) / 9;
			p->y[2][2] = (-4 * p->y[3][3] + 6 * (p->y[3][2] + p->y[2][3])
					- 2 * (p->y[3][0] + p->y[0][3])
					+ 3 * (p->y[0][2] + p->y[2][0]) - p->y[0][0]) / 9;
		}
	}

	shading = new GfxPatchMeshShading(typeA, patchesA, nPatchesA, funcsA,
			nFuncsA);
	if (!shading->init(res, dict, out, state)) {
		delete shading;
		return nullptr;
...
void GfxPatchMeshShading::getParameterizedColor(double t,
		GfxColor *color) const {
	double out[gfxColorMaxComps] = { };
...
GfxImageColorMap::GfxImageColorMap(int bitsA, Object *decode,
	...
	for (k = 0; k < nComps; ++k) {
		...
		for (i = 0; i <= maxPixel; ++i) {
			lookup[k][i] = dblToCol(
					decodeLow[k] + (i * decodeRange[k]) / maxPixel);
		}
	}
	...
	case csIndexed:
		...
		if (colorSpace2->useGetGrayLine() || colorSpace2->useGetRGBLine()
				|| colorSpace2->useGetCMYKLine()
				|| colorSpace2->useGetDeviceNLine()) {
		...
		for (k = 0; k < nComps2; ++k) {
			...
			for (i = 0; i <= maxPixel; ++i) {
				j =
						(int) (decodeLow[0] + (i * decodeRange[0]) / maxPixel
								+ 0.5);
				if (j < 0) {
					j = 0;
				} else if (j > indexHigh) {
	...
	default:
		if (colorSpace->useGetGrayLine() || colorSpace->useGetRGBLine()
				|| colorSpace->useGetCMYKLine()
				|| colorSpace->useGetDeviceNLine()) {
...
void GfxImageColorMap::getGrayLine(unsigned char *in, unsigned char *out,
		int length) {
	int i, j;
...
void GfxImageColorMap::getRGBLine(unsigned char *in, unsigned int *out,
		int length) {
	int i, j;
	unsigned char *inp, *tmp_line;
...
void GfxImageColorMap::getRGBLine(unsigned char *in, unsigned char *out,
		int length) {
	int i, j;
	unsigned char *inp, *tmp_line;
...
void GfxImageColorMap::getRGBXLine(unsigned char *in, unsigned char *out,
		int length) {
	int i, j;
	unsigned char *inp, *tmp_line;
...
void GfxImageColorMap::getCMYKLine(unsigned char *in, unsigned char *out,
		int length) {
	int i, j;
	unsigned char *inp, *tmp_line;
...
void GfxImageColorMap::getDeviceNLine(unsigned char *in, unsigned char *out,
		int length) {
	unsigned char *inp, *tmp_line;
...
void GfxImageColorMap::getColor(const unsigned char *x, GfxColor *color) {
	...
	for (i = 0; i < nComps; ++i) {
		color->c[i] = dblToCol(
				decodeLow[i] + (x[i] * decodeRange[i]) / maxPixel);
	}
}
...
void GfxPath::lineTo(double x, double y) {
	if (justMoved || (n > 0 && subpaths[n - 1]->isClosed())) {
		if (n >= size) {
			size *= 2;
			subpaths = (GfxSubpath**) greallocn(subpaths, size,
					sizeof(GfxSubpath*));
		}
		if (justMoved) {
...
GfxState::ReusablePathIterator::ReusablePathIterator(GfxPath *pathA) :
		path(pathA), subPathOff(0), coordOff(0), numCoords(0), curSubPath(
				nullptr) {
...
GfxState::GfxState(const GfxState *state, bool copyPath) {
	...
	lastRenderInfo = state->lastRenderInfo;
...
void GfxState::setDisplayProfile(
		const GfxLCMSProfilePtr &localDisplayProfileA) {
	...
	if (localDisplayProfile) {
		...
		localDisplayPixelType = getCMSColorSpaceType(
				cmsGetColorSpace(localDisplayProfile.get()));
		nChannels = getCMSNChannels(
				cmsGetColorSpace(localDisplayProfile.get()));
		...
		if ((transform = cmsCreateTransform(XYZProfile.get(), TYPE_XYZ_DBL,
			...
			XYZ2DisplayTransformRelCol = std::make_shared<GfxColorTransform>(
					transform, INTENT_RELATIVE_COLORIMETRIC, PT_XYZ,
		...
		if ((transform = cmsCreateTransform(XYZProfile.get(), TYPE_XYZ_DBL,
			...
			XYZ2DisplayTransformAbsCol = std::make_shared<GfxColorTransform>(
		...
		if ((transform = cmsCreateTransform(XYZProfile.get(), TYPE_XYZ_DBL,
			...
			XYZ2DisplayTransformSat = std::make_shared<GfxColorTransform>(
					transform, INTENT_SATURATION, PT_XYZ,
					localDisplayPixelType);
		}
		...
		if ((transform = cmsCreateTransform(XYZProfile.get(), TYPE_XYZ_DBL,
			...
			XYZ2DisplayTransformPerc = std::make_shared<GfxColorTransform>(
					transform, INTENT_PERCEPTUAL, PT_XYZ,
					localDisplayPixelType);
		}
	}
}
...
double GfxState::inverseTransformWidth(double w) const {
	double x, y;
...
void GfxState::getFontTransMat(double *m11, double *m12, double *m21,
		double *m22) const {
...
void GfxState::setCTM(double a, double b, double c, double d, double e,
		double f) {
	ctm[0] = a;
	ctm[1] = b;
	ctm[2] = c;
