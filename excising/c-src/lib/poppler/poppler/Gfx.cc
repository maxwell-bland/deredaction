...
const Operator Gfx::opTab[] = { { "\"", 3, { tchkNum, tchkNum, tchkString },
		&Gfx::opMoveSetShowText }, { "'", 1, { tchkString },
		&Gfx::opMoveShowText }, { "B", 0, { tchkNone }, &Gfx::opFillStroke }, {
		"B*", 0, { tchkNone }, &Gfx::opEOFillStroke }, { "BDC", 2, { tchkName,
		tchkProps }, &Gfx::opBeginMarkedContent }, { "BI", 0, { tchkNone },
		&Gfx::opBeginImage }, { "BMC", 1, { tchkName },
		&Gfx::opBeginMarkedContent },
		{ "BT", 0, { tchkNone }, &Gfx::opBeginText }, { "BX", 0, { tchkNone },
				&Gfx::opBeginIgnoreUndef }, { "CS", 1, { tchkName },
				&Gfx::opSetStrokeColorSpace }, { "DP", 2,
				{ tchkName, tchkProps }, &Gfx::opMarkPoint }, { "Do", 1, {
				tchkName }, &Gfx::opXObject }, { "EI", 0, { tchkNone },
				&Gfx::opEndImage }, { "EMC", 0, { tchkNone },
				&Gfx::opEndMarkedContent }, { "ET", 0, { tchkNone },
				&Gfx::opEndText }, { "EX", 0, { tchkNone },
				&Gfx::opEndIgnoreUndef },
		{ "F", 0, { tchkNone }, &Gfx::opFill }, { "G", 1, { tchkNum },
				&Gfx::opSetStrokeGray }, { "ID", 0, { tchkNone },
				&Gfx::opImageData },
		{ "J", 1, { tchkInt }, &Gfx::opSetLineCap }, { "K", 4, { tchkNum,
				tchkNum, tchkNum, tchkNum }, &Gfx::opSetStrokeCMYKColor }, {
				"M", 1, { tchkNum }, &Gfx::opSetMiterLimit }, { "MP", 1, {
				tchkName }, &Gfx::opMarkPoint }, { "Q", 0, { tchkNone },
				&Gfx::opRestore }, { "RG", 3, { tchkNum, tchkNum, tchkNum },
				&Gfx::opSetStrokeRGBColor }, { "S", 0, { tchkNone },
				&Gfx::opStroke }, { "SC", -4, { tchkNum, tchkNum, tchkNum,
				tchkNum }, &Gfx::opSetStrokeColor }, { "SCN", -33, { tchkSCN,
...
static inline bool isSameGfxColor(const GfxColor& colorA,
																	const GfxColor& colorB, unsigned int nComps,
																	double delta) {
...
GfxResources::~GfxResources() { delete fonts; }
...
GfxShading* GfxResources::lookupShading(const char* name, OutputDev* out,
																				GfxState* state) {
	GfxResources* resPtr;
	GfxShading* shading;
...
Gfx::Gfx(PDFDoc* docA, OutputDev* outA, int pageNum, Dict* resDict, double hDPI,
				 ...
				 int rotate, bool (*abortCheckCbkA)(void* data),
...
Gfx::Gfx(PDFDoc* docA, OutputDev* outA, Dict* resDict, const PDFRectangle* box,
				 const PDFRectangle* cropBox, bool (*abortCheckCbkA)(void* data),
			...
			if (firstElement.isDict()) {
				...
				if (profile.isStream()) {
					...
					unsigned char* profBuf =
							iccStream->toUnsignedChars(&length, 65536, 65536);
					auto hp =
							make_GfxLCMSProfilePtr(cmsOpenProfileFromMem(profBuf, length));
					if (!hp) {
						error(errSyntaxWarning, -1,
									"read ICCBased color space profile error");
					} else {
...
Goffset Gfx::getPos() { return parser ? parser->getPos() : -1; }
...
void Gfx::opConcat(Object args[], int numArgs) {
	state->concatCTM(args[0].getNum(), args[1].getNum(), args[2].getNum(),
									 args[3].getNum(), args[4].getNum(), args[5].getNum());
	out->updateCTM(state, args[0].getNum(), args[1].getNum(), args[2].getNum(),
								 args[3].getNum(), args[4].getNum(), args[5].getNum());
	fontChanged = true;
}
  ...
  obj2.free();
		...
		if (obj2.isName("None")) {
			...
			if (obj3.isStream()) {
				...
				if (obj4.isDict()) {
					...
					const bool isolated =
							obj4.dictLookup("I").getBoolWithDefaultValue(false);
					const bool knockout =
							obj4.dictLookup("K").getBoolWithDefaultValue(false);
					...
					doSoftMask(&obj3, alpha, blendingColorSpace, isolated, knockout,
										 funcs[0], &backdropColor);
	...
	if (obj2.isArray()) {
		...
		if (obj2.arrayGetLength() == 2) {
			...
			error(errSyntaxError, getPos(),
						"Number of args mismatch for /Font in ExtGState");
		}
	}
	obj2 = obj1.dictLookup("LW");
	if (obj2.isNum()) {
	...
	if (obj2.isArray()) {
		if (obj2.arrayGetLength() == 2) {
			...
			error(errSyntaxError, getPos(),
						"Number of args mismatch for /D in ExtGState");
		}
	}
...
void Gfx::doSoftMask(Object* str, bool alpha, GfxColorSpace* blendingColorSpace,
	...
	if (state->getFillColorSpace()->getMode() == csPattern) {
		...
		if (numArgs > 0) {
			if (args[numArgs - 1].isName() &&
					(pattern =
							 res->lookupPattern(args[numArgs - 1].getName(), out, state))) {
		...
		if (shouldDrawPattern) {
			if (out->useTilingPatternFill() &&
					out->tilingPatternFill(
							state, this, catalog, tPat->getContentStream(), tPat->getMatrix(),
							tPat->getPaintType(), tPat->getTilingType(), tPat->getResDict(),
							m1, tPat->getBBox(), xi0, yi0, xi1, yi1, xstep, ystep)) {
				// do nothing
			} else {
				...
				for (yi = yi0; yi < yi1; ++yi) {
					for (xi = xi0; xi < xi1; ++xi) {
						...
						drawForm(tPat->getContentStream(), tPat->getResDict(), m1,
										 tPat->getBBox());
					}
				}
...
void Gfx::doShadingPatternFill(GfxShadingPattern* sPat, bool stroke,
															 bool eoFill, bool text) {
	GfxShading* shading;
	GfxState* savedState;
	const double *ctm, *btm, *ptm;
	...
	while (i < axialMaxSplits) {
		...
		while (j > i + 1) {
			...
			if (isSameGfxColor(color1, color0, nComps, axialColorDelta)) {
				...
				// is making sure use the exact bboxIntersections[] value as one of the
				// used ta[] values
				if (!doneBBox1 && ta[i] < bboxIntersections[1] &&
						ta[j] > bboxIntersections[1]) {
					int teoricalj = (int)((bboxIntersections[1] - tMin) * axialMaxSplits /
																(tMax - tMin));
					if (teoricalj <= i)
						teoricalj = i + 1;
					if (teoricalj < j) {
				...
				if (!doneBBox2 && ta[i] < bboxIntersections[2] &&
						ta[j] > bboxIntersections[2]) {
					int teoricalj = (int)((bboxIntersections[2] - tMin) * axialMaxSplits /
																(tMax - tMin));
					if (teoricalj <= i)
						teoricalj = i + 1;
					if (teoricalj < j) {
...
void Gfx::doRadialShFill(GfxRadialShading* shading) {
	...
	if (x0 == x1 && y0 == y1) {
		...
		const double theta_aux =
				sqrt((x0 - xz) * (x0 - xz) + (y0 - yz) * (y0 - yz));
	...
	while (ia < radialMaxSplits) {
		...
		while (ib - ia > 1) {
			if (isSameGfxColor(colorB, colorA, nComps, radialColorDelta)) {
				...
				// same color does not mean all the areas in between have the same color
				// too
				int ic = ia + 1;
				for (; ic <= ib; ic++) {
					const double sc =
							sMin + ((double)ic / (double)radialMaxSplits) * (sMax - sMin);
...
void Gfx::doGouraudTriangleShFill(GfxGouraudTriangleShading* shading) {
	...
	if (shading->isParameterized()) {
		...
		const double refineColorThreshold =
...
void Gfx::gouraudFillTriangle(double x0, double y0, GfxColor* color0, double x1,
...
void Gfx::gouraudFillTriangle(double x0, double y0, double color0, double x1,
	...
	if (isFineEnough || depth == gouraudMaxDepth) {
		...
		gouraudFillTriangle(x01, y01, color01, x1, y1, color1, x12, y12, color12,
												refineColorThreshold, depth, shading, path);
		gouraudFillTriangle(x01, y01, color01, x12, y12, color12, x20, y20, color20,
												refineColorThreshold, depth, shading, path);
		gouraudFillTriangle(x20, y20, color20, x12, y12, color12, x2, y2, color2,
												refineColorThreshold, depth, shading, path);
	}
...
void Gfx::doPatchMeshShFill(GfxPatchMeshShading* shading) {
	...
	for (i = 0; i < shading->getNPatches(); ++i) {
		fillPatch(shading->getPatch(i), colorComps,
							shading->isParameterized() ? 1 : colorComps, refineColorThreshold,
							start, shading);
	}
}

void Gfx::fillPatch(const GfxPatch* patch, int colorComps, int patchColorComps,
...
#ifdef KERNING_DEBUG
	printf("Gfx::opSetTextMatrix\n");
#endif
	state->setTextMat(args[0].getNum(), args[1].getNum(), args[2].getNum(),
										args[3].getNum(), args[4].getNum(), args[5].getNum());
	state->textMoveTo(0, 0);
	out->updateTextMat(state);
	out->updateTextPos(state);
...
//------------------------------------------------------------------------
void Gfx::recordNewTextShow() {
	auto lr = out->lastRender();
	if (lr && state) {
		auto lastS = lr->postState.get();
		if (lastS != nullptr)
			if (lastS->getCurY() == state->getCurY())
				out->recordRender(GfxTextRender::TJ, '0', lastS->getCurX(),
													state->getCurY(), 0, 0, 0,
													state->getCurX() - lastS->getCurX(), 0, state);
	}
}

void Gfx::opShowText(Object args[], int numArgs) {
#ifdef KERNING_DEBUG
	printf("Gfx::opShowText\n");
#endif
	if (!state->getFont()) {
...
void Gfx::injectTextShift(int wMode, const double num) {
	// this uses the absolute value of the font size to match
	// Acrobat's behavior
	double shiftAmt = -num * 0.001 * state->getFontSize();
	if (wMode) {
		out->recordRender(GfxTextRender::Shift, num, state->getCurX(),
											state->getCurY(), 0, shiftAmt, 0, 0, shiftAmt, state);
		state->textShift(0, shiftAmt);
	} else {
		auto preState = state->copy(true);
		state->textShift(shiftAmt * state->getHorizScaling(), 0);
		out->recordRender(GfxTextRender::Shift, num, preState->getCurX(),
											state->getCurY(), 0, 0, 0,
											state->getCurX() - preState->getCurX(), 0, preState);
		delete preState;
		auto ri = out->lastRender();
		ri->postState.reset(state->copy(true));
	}
}

void Gfx::opShowSpaceText(Object args[], int numArgs) {
#ifdef KERNING_DEBUG
	printf("Gfx::opShowSpaceText\n");
#endif
	Array* a;
	int wMode;
	int i;
	...
	for (i = 0; i < a->getLength(); ++i) {
		...
		if (obj.isNum()) {
			// this uses the absolute value of the font size to match
			// Acrobat's behavior
			injectTextShift(wMode, obj.getNum());
...
double Gfx::scaleCharacterDeltas(int wMode, double dx, int n, const char* p,
																 double& dy) {
	if (wMode) {
...
void Gfx::doShowText(const GooString* s) {
	...
	if (font->getType() == fontType3 && out->interpretType3Chars()) {
		...
		len = s->getLength();
		while (len > 0) {
			n = font->getNextChar(p, len, &code, &u, &uLen, &dx, &dy, &originX,
														&originY);
			dx = dx * state->getFontSize() + state->getCharSpace();
			if (n == 1 && *p == ' ') {
				dx += state->getWordSpace();
					...
					if (refNum != -1) {
						if (charProcDrawing.find(refNum) == charProcDrawing.end()) {
							...
							error(errSyntaxError, -1,
										"CharProc wants to draw a CharProc that is already being "
										"drawn");
						}
					}
					if (displayCharProc) {
			...
			// Call updateCTM with the identity transformation.  That way, the CTM is
			// unchanged, but any side effect that the method may have is triggered.
			// This is the case, in particular, for the Splash backend.
			out->updateCTM(state, 1, 0, 0, 1, 0, 0);
			p += n;
			len -= n;
		}
		...
		while (len > 0) {
			n = font->getNextChar(p, len, &code, &u, &uLen, &dx, &dy, &originX,
														&originY);
			dx = scaleCharacterDeltas(wMode, dx, n, p, dy);
			state->textTransformDelta(dx, dy, &tdx, &tdy);
		...
		nChars = nSpaces = 0;
		while (len > 0) {
			n = font->getNextChar(p, len, &code, &u, &uLen, &dx2, &dy2, &originX,
														&originY);
			dx += dx2;
			dy += dy2;
			if (n == 1 && *p == ' ') {
...
void Gfx::doImage(Object* ref, Stream* str, bool inlineImg) {
	...
	if (ref) {
		...
		if (catalog->getOptContentConfig() &&
				!catalog->getOptContentConfig()->optContentIsVisible(&objOC)) {
			return;
		}
	}
		...
		if (smaskObj.isStream()) {
			...
			if (obj1.isArray()) {
				if (obj1.getArray()->getLength() != colorSpace->getNComps()) {
					error(errSyntaxError, -1,
								"Matte entry should have {0:d} components but has {1:d}",
								colorSpace->getNComps(), obj1.getArray()->getLength());
				} else if (maskWidth != width || maskHeight != height) {
					error(errSyntaxError, -1,
								"Softmask with matte entry {0:d} x {1:d} must have same "
								"geometry as the image {2:d} x {3:d}",
					...
					for (i = 0; i < colorSpace->getNComps(); i++) {
						...
						if (!obj2.isNum()) {
							error(
									errSyntaxError, -1,
									"Matte entry {0:d} should be a number but it's of type {1:d}",
									i, obj2.getType());
			...
			// color key mask
			for (i = 0; i < maskObj.arrayGetLength() && i < 2 * gfxColorMaxComps;
					 ++i) {
				...
				if (obj1.isInt()) {
					...
					error(errSyntaxError, -1,
								"Mask entry should be an integer but it's a real, trying to "
								"use it");
					maskColors[i] = (int)obj1.getReal();
				} else {
					error(errSyntaxError, -1,
								"Mask entry should be an integer but it's of type {0:d}",
								obj1.getType());
					goto err1;
				}
			}
...
void Gfx::drawForm(Object* str, Dict* resDict, const double* matrix,
									 const double* bbox, bool transpGroup, bool softMask,
									 GfxColorSpace* blendingColorSpace, bool isolated,
									 bool knockout, bool alpha, Function* transferFunc,
									 GfxColor* backdropColor) {
	Parser* oldParser;
	GfxState* savedState;
	...
	if (stateBefore != state) {
		if (state->isParentState(stateBefore)) {
			error(errSyntaxError, -1,
						"There's a form with more q than Q, trying to fix");
...
void Gfx::opSetCacheDevice(Object args[], int numArgs) {
	out->type3D1(state, args[0].getNum(), args[1].getNum(), args[2].getNum(),
							 args[3].getNum(), args[4].getNum(), args[5].getNum());
}
...
Gfx::gooStringToUnicode(const GooString* encoded) {
	const Unicode* u = nullptr;
	CharCode code;
	int len, n, uLen;
	double dx, dy;
	...
	// The encoding of strings in PDF can be file-defined, with
	// variable lengths
	p = encoded->c_str();
...
std::vector<CharCode> Gfx::gooStringToCharCode(const GooString* encoded) {
	const Unicode* u = nullptr;
	CharCode code;
	int len, n, uLen;
	double dx, dy;
...
GooString* Gfx::ccVectoToGooString(const std::vector<CharCode>& v2,
																	 bool useCID) {
...
std::vector<GfxRenderInfo *> Gfx::injectGuess(std::wstring guess) {
  std::vector<CharCode> ccs;
  for (auto chr : guess) {
	  ccs.push_back((CharCode) chr);
  }
  GooString *s =
      ccVectoToGooString(ccs, state->getFont()->getType() >= fontCIDType0);

  if (!s->getLength()) {
    std::wcerr << "Failed conversion of guess to GooString: " << guess
               << std::endl;
    delete s;
    out->resetRenderInfo();
    std::vector<GfxRenderInfo *> vect;
    return vect;
  }
  ...
  for (int i = 0; i < s->getLength(); i++) {
    vect.push_back(out->popLastRender());
  }
  out->resetRenderInfo();
  delete s;
  return vect;
