...
public:
	...
	void init(double xx, double yx, double xy, double yy, double x0,
			double y0) {
		m[0] = xx;
		m[1] = yx;
		m[2] = xy;
...
public:
	...
	int getInputPixelType() const {
		return inputPixelType;
	}
	int getTransformPixelType() const {
		return transformPixelType;
	}
private:
	GfxColorTransform() {
	}
...
public:
	...
	static GfxColorSpace* parse(GfxResources *res, Object *csObj,
			OutputDev *out, GfxState *state, int recursion = 0);
	...
	virtual void getGrayLine(unsigned char* /*in*/, unsigned char* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getGrayLine this should not happen");
	}
	virtual void getRGBLine(unsigned char* /*in*/, unsigned int* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getRGBLine (first variant) this should not happen");
	}
	virtual void getRGBLine(unsigned char* /*in*/, unsigned char* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getRGBLine (second variant) this should not happen");
	}
	virtual void getRGBXLine(unsigned char* /*in*/, unsigned char* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getRGBXLine this should not happen");
	}
	virtual void getCMYKLine(unsigned char* /*in*/, unsigned char* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getCMYKLine this should not happen");
	}
	virtual void getDeviceNLine(unsigned char* /*in*/, unsigned char* /*out*/,
			int /*length*/) {
		error(errInternal, -1,
				"GfxColorSpace::getDeviceNLine this should not happen");
	}
	...
	virtual bool useGetRGBLine() const {
		return false;
	}
	// Does this ColorSpace support getGrayLine?
	virtual bool useGetGrayLine() const {
		return false;
	}
	// Does this ColorSpace support getCMYKLine?
	virtual bool useGetCMYKLine() const {
		return false;
	}
	// Does this ColorSpace support getDeviceNLine?
	virtual bool useGetDeviceNLine() const {
		return false;
	}
	...
	virtual bool isNonMarking() const {
		return false;
	}

	// Return the color space's overprint mask.
	unsigned int getOverprintMask() const {
...
public:
	...
	bool useGetGrayLine() const override {
		return true;
	}
...
private:
	...
	void getXYZ(const GfxColor *color, double *pX, double *pY,
			double *pZ) const;
#ifdef USE_CMS
...
public:
	...
	static GfxColorSpace* parse(Array *arr, OutputDev *out, GfxState *state,
			int recursion);
...
public:
	...
	static GfxColorSpace* parse(GfxResources *res, Array *arr, OutputDev *out,
...
public:
	...
	bool isNonMarking() const override {
		return nonMarking;
	}
...
private:
	...
	GfxSeparationColorSpace(GooString *nameA, GfxColorSpace *altA,
			Function *funcA, bool nonMarkingA, unsigned int overprintMaskA,
			int *mappingA);
...
public:
	...
	GfxDeviceNColorSpace(int nCompsA, std::vector<std::string> &&namesA,
			...
			std::vector<GfxSeparationColorSpace*> *sepsCS);
	...
	const std::string& getColorantName(int i) const {
		return names[i];
	}
	GfxColorSpace* getAlt() {
		return alt;
...
private:
	...
	GfxDeviceNColorSpace(int nCompsA, const std::vector<std::string> &namesA,
			GfxColorSpace *alt, Function *func,
			std::vector<GfxSeparationColorSpace*> *sepsCSA, int *mappingA,
...
public:
	...
	static GfxPattern* parse(GfxResources *res, Object *obj, OutputDev *out,
	...
	int getPatternRefNum() const {
		return patternRefNum;
	}
...
public:
	...
	int getPaintType() const {
		return paintType;
	}
	int getTilingType() const {
		return tilingType;
	}
	const double* getBBox() const {
		return bbox;
	}
...
private:
	...
	GfxTilingPattern(int paintTypeA, int tilingTypeA, const double *bboxA,
			double xStepA, double yStepA, const Object *resDictA,
			const double *matrixA, const Object *contentStreamA,
			int patternRefNumA);
...
public:
	...
	static GfxShadingPattern* parse(GfxResources *res, Object *patObj,
...
public:
	...
	static GfxShading* parse(GfxResources *res, Object *obj, OutputDev *out,
			GfxState *state);
	...
	const GfxColor* getBackground() const {
		return &background;
	}
	bool getHasBackground() const {
		return hasBackground;
	}
	void getBBox(double *xMinA, double *yMinA, double *xMaxA,
			double *yMaxA) const {
		*xMinA = bbox_xMin;
		*yMinA = bbox_yMin;
		*xMaxA = bbox_xMax;
...
public:
	...
	GfxUnivariateShading(int typeA, double t0A, double t1A, Function **funcsA,
	...
	double getDomain0() const {
		return t0;
	}
	double getDomain1() const {
		return t1;
	}
	bool getExtend0() const {
		return extend0;
	}
	bool getExtend1() const {
		return extend1;
	}
	int getNFuncs() const {
		return nFuncs;
	}
	const Function* getFunc(int i) const {
		return funcs[i];
	}
	...
	void setupCache(const Matrix *ctm, double xMin, double yMin, double xMax,
			double yMax);

	virtual void getParameterRange(double *lower, double *upper, double xMin,
			double yMin, double xMax, double yMax) = 0;
...
public:
	...
	GfxFunctionShading(double x0A, double y0A, double x1A, double y1A,
			const double *matrixA, Function **funcsA, int nFuncsA);
	GfxFunctionShading(const GfxFunctionShading *shading);
	~GfxFunctionShading() override;

	static GfxFunctionShading* parse(GfxResources *res, Dict *dict,
...
public:
	...
	void getParameterRange(double *lower, double *upper, double xMin,
			double yMin, double xMax, double yMax) override;
...
public:
	...
	GfxGouraudTriangleShading(int typeA, GfxGouraudVertex *verticesA,
			int nVerticesA, int (*trianglesA)[3], int nTrianglesA,
			Function **funcsA, int nFuncsA);
	GfxGouraudTriangleShading(const GfxGouraudTriangleShading *shading);
	~GfxGouraudTriangleShading() override;

	static GfxGouraudTriangleShading* parse(GfxResources *res, int typeA,
	...
	void getTriangle(int i, double *x0, double *y0, GfxColor *color0,
			double *x1, double *y1, GfxColor *color1, double *x2, double *y2,
			GfxColor *color2);
	...
	void getTriangle(int i, double *x0, double *y0, double *color0, double *x1,
			double *y1, double *color1, double *x2, double *y2, double *color2);
...
public:
	...
	static GfxPatchMeshShading* parse(GfxResources *res, int typeA, Dict *dict,
			Stream *str, OutputDev *out, GfxState *state);

	GfxShading* copy() const override;

	int getNPatches() const {
		return nPatches;
	}
	const GfxPatch* getPatch(int i) const {
		return &patches[i];
	}
...
public:
	...
	// Get decode table.
	double getDecodeLow(int i) const {
		return decodeLow[i];
	}
	double getDecodeHigh(int i) const {
	...
	const GfxColor* getMatteColor() const {
		return (useMatte) ? &matteColor : nullptr;
	}
private:
...
public:
	...
	bool getCurve(int i) const {
		return curve[i];
	}

	void setX(int i, double a) {
		x[i] = a;
	}
	void setY(int i, double a) {
		y[i] = a;
	}
...
public:
	...
	// Get subpaths.
	int getNumSubpaths() const {
		return n;
	}
	GfxSubpath* getSubpath(int i) {
		return subpaths[i];
	}
	const GfxSubpath* getSubpath(int i) const {
		return subpaths[i];
	}
...
public:
	...
	// Accessors.
	void setRenderInfo(GfxRenderInfo * ri) {
		lastRenderInfo = ri;
	}

	GfxRenderInfo * getRenderInfo() {
	...
	void getCTM(Matrix *m) const {
		memcpy(m->m, ctm, sizeof m->m);
	}
	double getX1() const {
		return px1;
	}
	...
	void setPageWidth(double pw) {
		pageWidth = pw;
	}
	void setPageHeight(double ph) {
		pageHeight = ph;
	}
	...
	GfxBlendMode getBlendMode() const {
		return blendMode;
	}
	...
	const char* getRenderingIntent() const {
		return renderingIntent;
	}
	const GfxPath* getPath() const {
		return path;
	}
	...
	void getClipBBox(double *xMin, double *yMin, double *xMax,
			double *yMax) const {
		*xMin = clipXMin;
		*yMin = clipYMin;
		*xMax = clipXMax;
	...
	void getUserClipBBox(double *xMin, double *yMin, double *xMax,
			double *yMax) const;
	...
	void textTransformDelta(double x1, double y1, double *x2,
			double *y2) const {
	...
	void getFontTransMat(double *m11, double *m12, double *m21,
			double *m22) const;
	...
	void setFillColor(const GfxColor *color) {
		fillColor = *color;
	}
	void setStrokeColor(const GfxColor *color) {
		strokeColor = *color;
	}
	...
	void setStrokeAdjust(bool sa) {
		strokeAdjust = sa;
	}
	void setAlphaIsShape(bool ais) {
		alphaIsShape = ais;
	}
	void setTextKnockout(bool tk) {
		textKnockout = tk;
	}
	void setFont(GfxFont *fontA, double fontSizeA);
	void setTextMat(double a, double b, double c, double d, double e,
			double f) {
		textMat[0] = a;
		textMat[1] = b;
		textMat[2] = c;
...
private:
	...
	/* Saved by Redaction library to sanity check injection points */
	GfxRenderInfo *lastRenderInfo;
