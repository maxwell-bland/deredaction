...
// Copyright (C) 2007, 2008, 2011, 2013 Carlos Garcia Campos
// <carlosgc@gnome.org> Copyright (C) 2007, 2017 Adrian Johnson
// <ajohnson@redneon.com> Copyright (C) 2008, 2010, 2015, 2016, 2018, 2019
// Albert Astals Cid <aacid@kde.org> Copyright (C) 2010 Brian Ewins
// <brian.ewins@gmail.com> Copyright (C) 2012, 2013, 2015, 2016 Jason Crain
// <jason@aquaticape.us> Copyright (C) 2013 Thomas Freitag
// <Thomas.Freitag@alfa.de> Copyright (C) 2018 Klarälvdalens Datakonsult AB, a
// KDAB Group company, <info@kdab.com>. Work sponsored by the LiMux project of
// the city of Munich Copyright (C) 2018 Sanchit Anand <sanxchit@gmail.com>
...
public:
	...
	void addChar(const GfxState *state, TextFontInfo *fontA, double x, double y,
							 double dx, double dy, int charPosA, int charLen, CharCode c,
	...
	bool addCombining(const GfxState *state, TextFontInfo *fontA,
...
private:
	...
	int visitDepthFirst(TextBlock *blkList, int pos1, TextBlock **sorted,
											int sortPos, bool *visited);
	int visitDepthFirst(TextBlock *blkList, int pos1, TextBlock **sorted,
											int sortPos, bool *visited, TextBlock **cache,
											int cacheSize);
...
public:
	...
	void addChar(GfxState *state, OutputDev *out, double x, double y, double dx,
							 double dy, CharCode c, int nBytes, const Unicode *u, int uLen);
	...
	// <xMax>,<yMax>.
	bool findText(const Unicode *s, int len, bool startAtTop, bool stopAtBottom,
	...
	bool findText(const Unicode *s, int len, bool startAtTop, bool stopAtBottom,
								bool startAtLast, bool stopAtLast, bool caseSensitive,
								bool ignoreDiacritics, bool backward, bool wholeWord,
	...
	std::vector<TextWordSelection *> **
	getSelectionWords(const PDFRectangle *selection, SelectionStyle style,
										int *nLines);

	// Find a string by character position and length.  If found, sets
	// the text bounding rectangle and returns true; otherwise returns
	// false.
	bool findCharRange(int pos, int length, double *xMin, double *yMin,
										 double *xMax, double *yMax) const;

	// Dump contents of page to a file.
	void dump(void *outputStream, TextOutputFunc outputFunc, bool physLayout,
						EndOfLineKind textEOL, bool pageBreaks);
...
public:
	...
	void addChar(GfxState *state, OutputDev *dev, double x, double y, double dx,
							 double dy, CharCode c, int nBytes, const Unicode *u, int uLen);
	void begin(const GfxState *state, const GooString *text);
	void end(GfxState *state, OutputDev *dev);
...
public:
	...
	TextOutputDev(const char *fileName, bool physLayoutA, double fixedPitchA,
								bool rawOrderA, bool append, bool discardDiagA = false);
	...
	// content stream order.  If <discardDiag> is true, diagonal text
	// is removed from output.
	TextOutputDev(TextOutputFunc func, void *stream, bool physLayoutA,
								double fixedPitchA, bool rawOrderA, bool discardDiagA = false);
...
private:
  ...
  // ========== For Redaction ==========================
public:
  void recordRender(GfxTextRender gtr, double val, double x,
                              double y, double sp, double w,
                              int unicodeIndex, double o_dx, double o_dy,
                              GfxState *state) override {
    auto ri = new GfxRenderInfo(gtr, val, x, y, sp, w, unicodeIndex, o_dx, o_dy, state);
    renderInfo.push_back(ri);
  }

#define rndTh(x) (int) (x * 1000 + 0.5)

  /**
   * Gets the proper GfxState for a coordinate pair
   *
   * NOTE: Does not handle injecting to the beginning of a line!
   */
  GfxState* getRenderState(double x, double y) {
      for (auto ri : renderInfo) {
        if (rndTh(ri->y) == rndTh(y)) {
          if (rndTh(ri->x) == rndTh(x)) {
            return ri->state->copy(true);
          }
        }
    }

    std::cerr << "BUG: Could not locate Gfx state for given text!"
              << std::endl;
    exit(1);
  }

  GfxState* getFirstState() { return renderInfo.front()->state->copy(true); }

  GfxRenderInfo *lastRender() override {
    if (renderInfo.size())
      return renderInfo.back();
    else return nullptr;
  }

  GfxRenderInfo* popLastRender() override {
    if (renderInfo.size()) {
      auto ri = renderInfo.back();
      renderInfo.pop_back();
      return ri;
    }
  ...
  void printRenderInfo() override {
    if (renderInfo.size()) {
      for (auto r : renderInfo) {
        std::wcerr << (std::wstring)*r << std::endl;
      }
    }
  }

  std::vector<GfxRenderInfo*> getRenderInfo() {
    return renderInfo;
  }

private:
  std::vector<GfxRenderInfo*> renderInfo;
};
