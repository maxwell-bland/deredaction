...
TextFontInfo::TextFontInfo(const GfxState *state) {
	...
	fontName =
			(gfxFont && gfxFont->getName()) ?
					gfxFont->getName()->copy() : nullptr;
...
void TextWord::addChar(const GfxState *state, TextFontInfo *fontA, double x,
		double y, double dx, double dy, int charPosA, int charLen, CharCode c,
		Unicode u, const Matrix &textMatA) {
...
void TextWord::ensureCapacity(int capacity) {
	if (capacity > size) {
		...
		charcode = (CharCode*) greallocn(charcode, (size + 1),
				sizeof(CharCode));
...
static const struct CombiningTable combiningTable[] = { { 0x0060, 0x0300 }, // grave
...
bool TextWord::addCombining(const GfxState *state, TextFontInfo *fontA,
		double fontSizeA, double x, double y, double dx, double dy,
		int charPosA, int charLen, CharCode c, Unicode u,
		const Matrix &textMatA) {
	...
	if (cPrev != 0 && unicodeTypeAlphaNum(u)) {
		...
		maxScaledBaseDelta = (fontA->getAscent() - fontA->getDescent())
				* fontSizeA * combMaxBaseDelta;
...
void TextPool::addWord(TextWord *word) {
	...
	if (unlikely(
			wordBaseIdx <= INT_MIN + 128 || wordBaseIdx >= INT_MAX - 128)) {
	...
	if (minBaseIdx > maxBaseIdx) {
		...
		TextWord **newPool = (TextWord**) gmallocn_checkoverflow(
				maxBaseIdx - newMinBaseIdx + 1, sizeof(TextWord*));
		if (unlikely(!newPool)) {
		...
		TextWord **reallocatedPool = (TextWord**) greallocn(pool,
				newMaxBaseIdx - minBaseIdx + 1, sizeof(TextWord*),
				true /*checkoverflow*/, false /*free_pool*/);
		if (!reallocatedPool) {
	...
	if (words->next) {
		...
		if (words->len > 1 || words->next->len > 1) {
			...
			for (word0 = words->next, word1 = word0->next;
					word1 && minSpace > 0; word0 = word1, word1 = word0->next) {
				if (word1->len > 1) {
					minSpace = 0;
				}
...
int TextLineFrag::cmpXYColumnPrimaryRot(const void *p1, const void *p2) {
	...
	if (frag1->col
			< frag2->col
					+ (frag2->line->col[frag2->start + frag2->len]
							- frag2->line->col[frag2->start])
			&& frag2->col
					< frag1->col
							+ (frag1->line->col[frag1->start + frag1->len]
...
void TextBlock::coalesce(const UnicodeMap *uMap, double fixedPitch) {
					...
					if (word2->len == word0->len
						...
						case 2:
							found = fabs(word0->xMin - word2->xMin) < priDelta
									&& fabs(word0->xMax - word2->xMax)
											< priDelta
									&& fabs(word0->yMin - word2->yMin)
											< secDelta
									&& fabs(word0->yMax - word2->yMax)
											< secDelta;
							break;
						case 1:
						case 3:
	...
	while (true) {
		...
		while (true) {
			...
			overlap = false;
			for (baseIdx = minBaseIdx; !overlap && baseIdx <= maxBaseIdx;
					++baseIdx) {
				for (word0 = nullptr, word1 = pool->getPool(baseIdx); word1;
...
bool TextBlock::isBeforeByRule1(const TextBlock *blk1) {
	...
	case 2:
		overlap =
				((this->ExMin <= blk1->ExMin) && (blk1->ExMin <= this->ExMax))
	...
	case 3:
		overlap =
				((this->EyMin <= blk1->EyMin) && (blk1->EyMin <= this->EyMax))
...
int TextBlock::visitDepthFirst(TextBlock *blkList, int pos1, TextBlock **sorted,
		int sortPos, bool *visited, TextBlock **cache, int cacheSize) {
	int pos2;
  ...
  printf("visited: %d %.2f..%.2f %.2f..%.2f\n",
	...
	for (blk2 = blkList; blk2; blk2 = blk2->next) {
		...
		if (blk1->tableId >= 0 && blk1->tableId == blk2->tableId) {
			if (page->primaryLR) {
				if (blk2->xMax <= blk1->xMin && blk2->yMin <= blk1->yMax
						&& blk2->yMax >= blk1->yMin)
					before = true;
			} else {
				if (blk2->xMin >= blk1->xMax && blk2->yMin <= blk1->yMax
						&& blk2->yMax >= blk1->yMin)
					before = true;
			}
	  ...
	  printf("rule2: %.2f..%.2f %.2f..%.2f %.2f..%.2f %.2f..%.2f\n",
		...
		if (before) {
			...
			sortPos = blk2->visitDepthFirst(blkList, pos2, sorted, sortPos,
					visited, cache, cacheSize);
		}
	}
#if 0 // for debugging
...
void TextPage::addChar(GfxState *state, OutputDev *out, double x, double y,
		double dx, double dy, CharCode c, int nBytes, const Unicode *u,
		int uLen) {
  ...
  double o_dy = dy;
	...
	if (x1 + w1 < 0 || (x1 > state->getPageWidth() && x1 > state->getPageHeight()) ||
      y1 + h1 < 0 || (y1 > state->getPageHeight() && y1 > state->getPageWidth()) ||
      x1 != x1 || y1 != y1 || // IEEE way of checking for isnan
			w1 != w1 || h1 != h1) {
          out->recordRender(GfxTextRender::Sanity, 0, x, y,
                            dx2, dx, 0, o_dx, o_dy,
                            state);
          charPos += nBytes;
          return;
	...
	if (fabs(w1) < 3 && fabs(h1) < 3) {
		if (++nTinyChars > 50000) {
                  out->recordRender(GfxTextRender::Tinychar, 0, x, y,
                                    dx2, dx, 0, o_dx,
                                    o_dy, state);
                  charPos += nBytes;
                  return;
	...
	if (uLen == 1 && UnicodeIsWhitespace(u[0])) {
          out->recordRender(GfxTextRender::Space, u[0], x, y,
                            dx2, dx, 0, o_dx, o_dy,
                            state);
          charPos += nBytes;
          endWord();
		...
		// ignore null characters
                out->recordRender(GfxTextRender::Null, 0, x, y,
                                  dx2, dx, 0, o_dx,
                                  o_dy, state);
                charPos += nBytes;
		return;
	...
	if (mergeCombining && curWord && uLen == 1
			&& curWord->addCombining(state, curFont, curFontSize, x1, y1, w1,
					h1, charPos, nBytes, c, u[0], mat)) {
          out->recordRender(GfxTextRender::Merge, 0, x, y,
                            dx2, dx, 0, o_dx, o_dy,
                            state);
          charPos += nBytes;
          return;
	...
	if (uLen != 0) {
		...
		if ((curWord->rot == 0 && w1 < 0) || (curWord->rot == 1 && h1 < 0)
				|| (curWord->rot == 2 && w1 > 0)
				|| (curWord->rot == 3 && h1 > 0)) {
			endWord();
			beginWord(state);
		...
		h1 /= uLen;
		for (i = 0; i < uLen; ++i) {
			unsigned short val = u[i] & 0xffff;
			out->recordRender(GfxTextRender::Char, val, x, y, dx2,
                                  dx, i, o_dx, o_dy, state);
			curWord->addChar(state, curFont, x1 + i * w1, y1 + i * h1, w1, h1,
					charPos, nBytes, c, u[i], mat);
		}
	}
	charPos += nBytes;
}
...
void TextPage::addLink(int xMin, int yMin, int xMax, int yMax,
		AnnotLink *link) {
...
void TextPage::coalesce(bool physLayout, double fixedPitch, bool doHTML) {
		...
		for (const TextUnderline *underline : *underlines) {
			if (underline->horiz) {
				...
				if (pools[0]->minBaseIdx <= pools[0]->maxBaseIdx) {
					startBaseIdx = pools[0]->getBaseIdx(
							underline->y0 + minUnderlineGap);
					endBaseIdx = pools[0]->getBaseIdx(
							underline->y0 + maxUnderlineGap);
					for (j = startBaseIdx; j <= endBaseIdx; ++j) {
						for (word0 = pools[0]->getPool(j); word0;
								word0 = word0->next) {
							//~ need to check the y value against the word baseline
							if (underline->x0 < word0->xMin + underlineSlack
									&& word0->xMax - underlineSlack
											< underline->x1) {
				...
				if (pools[2]->minBaseIdx <= pools[2]->maxBaseIdx) {
					startBaseIdx = pools[2]->getBaseIdx(
							underline->y0 - maxUnderlineGap);
					endBaseIdx = pools[2]->getBaseIdx(
							underline->y0 - minUnderlineGap);
					for (j = startBaseIdx; j <= endBaseIdx; ++j) {
						for (word0 = pools[2]->getPool(j); word0;
								word0 = word0->next) {
							if (underline->x0 < word0->xMin + underlineSlack
									&& word0->xMax - underlineSlack
											< underline->x1) {
				...
				if (pools[1]->minBaseIdx <= pools[1]->maxBaseIdx) {
					startBaseIdx = pools[1]->getBaseIdx(
							underline->x0 - maxUnderlineGap);
					endBaseIdx = pools[1]->getBaseIdx(
							underline->x0 - minUnderlineGap);
					for (j = startBaseIdx; j <= endBaseIdx; ++j) {
						for (word0 = pools[1]->getPool(j); word0;
								word0 = word0->next) {
							if (underline->y0 < word0->yMin + underlineSlack
									&& word0->yMax - underlineSlack
											< underline->y1) {
								word0->underlined = true;
							}
						}
				...
				if (pools[3]->minBaseIdx <= pools[3]->maxBaseIdx) {
					...
					endBaseIdx = pools[3]->getBaseIdx(
							underline->x0 + maxUnderlineGap);
					for (j = startBaseIdx; j <= endBaseIdx; ++j) {
						for (word0 = pools[3]->getPool(j); word0;
								word0 = word0->next) {
							if (underline->y0 < word0->yMin + underlineSlack
									&& word0->yMax - underlineSlack
											< underline->y1) {
	...
	for (rot = 0; rot < 4; ++rot) {
		...
		while (true) {
			...
			for (baseIdx = poolMinBaseIdx + 1;
				...
				if (pool->getPool(baseIdx)->primaryCmp(
						pool->getPool(startBaseIdx)) < 0) {
					startBaseIdx = baseIdx;
				}
			}
  ...
  printf("primaryLR = %d\n", primaryLR);
	...
	if (physLayout && fixedPitch) {
		...
		if (blocks) {
			qsort(blocks, nBlocks, sizeof(TextBlock*),
  ...
  printf("PAGE\n");
	...
	for (blk1 = blkList; blk1; blk1 = blk1->next) {
		...
		for (blk2 = blkList; blk2; blk2 = blk2->next) {
			if (blk2 != blk1) {
				if (blk2->yMin <= blk1->yMax && blk2->yMax >= blk1->yMin
						&& blk2->xMin > blk1->xMax && blk2->xMin < bxMin0) {
					bxMin0 = blk2->xMin;
					fblk2 = blk2;
				} else if (blk2->xMin <= blk1->xMax && blk2->xMax >= blk1->xMin
						&& blk2->yMin > blk1->yMax && blk2->yMin < byMin0) {
					byMin0 = blk2->yMin;
					fblk3 = blk2;
				} else if (blk2->xMin > blk1->xMax && blk2->xMin < bxMin1
						&& blk2->yMin > blk1->yMax && blk2->yMin < byMin1) {
					bxMin1 = blk2->xMin;
					byMin1 = blk2->yMin;
					fblk4 = blk2;
				}
		...
		if (fblk2 != nullptr && fblk3 != nullptr && fblk4 != nullptr) {
			if (((fblk3->xMin <= fblk4->xMax && fblk3->xMax >= fblk4->xMin)
					...
					|| !(fblk4->xMin <= fblk2->xMax
							&& fblk4->xMax >= fblk2->xMin
							&& fblk4->yMin <= fblk3->yMax
							&& fblk4->yMax >= fblk3->yMin)) {
				fblk2 = nullptr;
				fblk3 = nullptr;
				fblk4 = nullptr;
			}
	...
	for (blk1 = blkList; blk1; blk1 = blk1->next) {
		if (blk1->tableId >= 0) {
			...
			envelopes[blk1->tableId].y1 =
					blk1->EyMin < envelopes[blk1->tableId].y1 ?
							blk1->EyMin : envelopes[blk1->tableId].y1;
			envelopes[blk1->tableId].y2 =
	...
	for (blk1 = blkList; blk1; blk1 = blk1->next) {
		if (!(blk1->tableId >= 0)) {
			...
			for (blk2 = blkList; blk2; blk2 = blk2->next) {
				...
				if (blk2->xMax > blk1->ExMax && blk2->xMax <= xMax
						&& blk2->yMin >= blk1->yMax) {
					blk1->ExMax = blk2->xMax;
				}

				if (blk2->xMin < blk1->ExMin && blk2->xMin >= xMin
						&& blk2->yMin >= blk1->yMax)
  ...
  fflush(stdout);
	...
	for (i = 0; i < nBlocks; i++) {
		...
		if (flow) {
			...
			if (blk1->secondaryDelta(blk) <= blkSpace && blk->isBelow(blk1)
...
bool TextPage::findText(const Unicode *s, int len, bool startAtTop,
		bool stopAtBottom, bool startAtLast, bool stopAtLast,
		bool caseSensitive, bool backward, bool wholeWord, double *xMin,
...
bool TextPage::findText(const Unicode *s, int len, bool startAtTop,
		bool stopAtBottom, bool startAtLast, bool stopAtLast,
		bool caseSensitive, bool ignoreDiacritics, bool backward,
		bool wholeWord, double *xMin, double *yMin, double *xMax,
		double *yMax) {
	TextBlock *blk;
	TextLine *line;
	...
	for (i = backward ? nBlocks - 1 : 0; backward ? i >= 0 : i < nBlocks;
			i += backward ? -1 : 1) {
		blk = blocks[i];
		...
		for (line = blk->lines; line; line = line->next) {
			...
			if (ignoreDiacritics) {
				if (!line->ascii_translation)
					unicodeToAscii7(line->normalized, line->normalized_len,
							&line->ascii_translation, &line->ascii_len,
							line->normalized_idx, &line->ascii_idx);
				if (line->ascii_len)
					m = line->ascii_len;
				else
					ignoreDiacritics = false;
...
GooString* TextPage::getText(double xMin, double yMin, double xMax, double yMax,
	...
	if (rawOrder) {
		...
		for (word = rawWords; word && word <= rawLastWord; word = word->next) {
			for (j = 0; j < word->getLength(); ++j) {
				...
				if (xMin <= gXMin && gXMax <= xMax && yMin <= gYMin
						&& gYMax <= yMax) {
					mbc_len = uMap->mapUnicode(*(word->getChar(j)), mbc,
							sizeof(mbc));
	...
	for (i = 0; i < nBlocks; ++i) {
		blk = blocks[i];
		if (xMin < blk->xMax && blk->xMin < xMax && yMin < blk->yMax
				&& blk->yMin < yMax) {
			for (line = blk->lines; line; line = line->next) {
				if (xMin < line->xMax && line->xMin < xMax && yMin < line->yMax
						&& line->yMin < yMax) {
					idx0 = idx1 = -1;
					switch (line->rot) {
					case 0:
						...
						if (yMin < y && y < yMax) {
							j = 0;
							while (j < line->len) {
								if (0.5 * (line->edge[j] + line->edge[j + 1])
										> xMin) {
									idx0 = j;
									break;
								}
					...
					if (idx0 >= 0 && idx1 >= 0) {
						if (nFrags == fragsSize) {
							fragsSize *= 2;
							frags = (TextLineFrag*) greallocn(frags, fragsSize,
									sizeof(TextLineFrag));
						}
	...
	if (nFrags > 0) {
		...
		for (i = 0; i < nFrags; ++i) {
			...
			col += dumpFragment(frag->line->text + frag->start, frag->len, uMap,
					s);
		}
...
public:
	...
	virtual void visitBlock(TextBlock *block, TextLine *begin, TextLine *end,
			const PDFRectangle *selection) = 0;
	virtual void visitLine(TextLine *line, TextWord *begin, TextWord *end,
			int edge_begin, int edge_end, const PDFRectangle *selection) = 0;
...
public:
	...
	void visitBlock(TextBlock *block, TextLine *begin, TextLine *end,
			const PDFRectangle *selection) override {
	}
	;
	void visitLine(TextLine *line, TextWord *begin, TextWord *end,
			int edge_begin, int edge_end, const PDFRectangle *selection)
					override;
...
TextSelectionDumper::TextSelectionDumper(TextPage *p) :
	...
	linesSize = 256;
	lines = (std::vector<TextWordSelection*>**) gmallocn(linesSize,
...
void TextSelectionDumper::finishLine() {
	if (nLines == linesSize) {
		linesSize *= 2;
		lines = (std::vector<TextWordSelection*>**) grealloc(lines,
				linesSize * sizeof(std::vector<TextWordSelection*>*));
	}
...
void TextSelectionDumper::visitLine(TextLine *line, TextWord *begin,
		TextWord *end, int edge_begin, int edge_end,
...
GooString* TextSelectionDumper::getText() {
	...
	for (i = 0; i < nLines; i++) {
		...
		for (std::size_t j = 0; j < lineWords->size(); j++) {
			...
			page->dumpFragment(sel->word->text + sel->begin,
					sel->end - sel->begin, uMap, text);
...
std::vector<TextWordSelection*>** TextSelectionDumper::takeWordList(
		int *nLinesOut) {
...
public:
	...
	std::vector<PDFRectangle*>* getRegion() {
		return list;
	}
...
TextSelectionSizer::TextSelectionSizer(TextPage *p, double s) :
		TextSelectionVisitor(p), scale(s) {
	list = new std::vector<PDFRectangle*>();
}

void TextSelectionSizer::visitLine(TextLine *line, TextWord *begin,
		TextWord *end, int edge_begin, int edge_end,
		const PDFRectangle *selection) {
	PDFRectangle *rect;
	...
	rect = new PDFRectangle(floor(x1 * scale), floor(y1 * scale),
			ceil(x2 * scale), ceil(y2 * scale));
	list->push_back(rect);
}

class TextSelectionPainter: public TextSelectionVisitor {
public:
	TextSelectionPainter(TextPage *page, double scale, int rotation,
			OutputDev *out, const GfxColor *box_color,
...
TextSelectionPainter::TextSelectionPainter(TextPage *p, double scale,
		int rotation, OutputDev *outA, const GfxColor *box_color,
		const GfxColor *glyph_colorA) :
		TextSelectionVisitor(p), out(outA), glyph_color(glyph_colorA) {
...
void TextSelectionPainter::visitLine(TextLine *line, TextWord *begin,
		TextWord *end, int edge_begin, int edge_end,
		const PDFRectangle *selection) {
	double x1, y1, x2, y2, margin;
...
void TextSelectionPainter::endPage() {
	...
	for (const TextWordSelection *sel : *selectionList) {
		...
		while (begin < sel->end) {
			...
			GooString *string = new GooString((char*) sel->word->charcode,
					fEnd - begin);
			out->beginString(state, string);

			for (int j = begin; j < fEnd; j++) {
				if (j != begin
						&& sel->word->charPos[j] == sel->word->charPos[j - 1])
					continue;
...
void TextWord::visitSelection(TextSelectionVisitor *visitor,
		const PDFRectangle *selection, SelectionStyle style) {
	int i, begin, end;
	double mid;
...
void TextBlock::visitSelection(TextSelectionVisitor *visitor,
	...
	for (p = this->lines; p; p = p->next) {
		count++;
		for (i = 0; i < 2; i++) {
			d = fmax(p->xMin - x[i], 0.0) + fmax(x[i] - p->xMax, 0.0)
					+ fmax(p->yMin - y[i], 0.0) + fmax(y[i] - p->yMax, 0.0);
...
void TextPage::visitSelection(TextSelectionVisitor *visitor,
	...
	for (flow = flows; flow; flow = flow->next) {
		for (blk = flow->blocks; blk; blk = blk->next) {
			...
			for (i = 0; i < 2; i++) {
				d = fmax(blk->xMin - x[i], 0.0) + fmax(x[i] - blk->xMax, 0.0)
						+ fmax(blk->yMin - y[i], 0.0)
						+ fmax(y[i] - blk->yMax, 0.0);
				if (!best_block[i] || d < best_d[i]
						|| (!blk->next && !flow->next
...
void TextPage::drawSelection(OutputDev *out, double scale, int rotation,
		const PDFRectangle *selection, SelectionStyle style,
		const GfxColor *glyph_color, const GfxColor *box_color) {
	TextSelectionPainter painter(this, scale, rotation, out, box_color,
			glyph_color);
...
std::vector<PDFRectangle*>* TextPage::getSelectionRegion(
		const PDFRectangle *selection, SelectionStyle style, double scale) {
...
std::vector<TextWordSelection*>** TextPage::getSelectionWords(
		const PDFRectangle *selection, SelectionStyle style, int *nLines) {
	TextSelectionDumper dumper(this);
...
bool TextPage::findCharRange(int pos, int length, double *xMin, double *yMin,
		...
		for (line = blk->lines; line; line = line->next) {
			for (word = line->words; word; word = word->next) {
				if (pos < word->charPos[word->len]
					...
					for (j0 = 0; j0 < word->len && pos >= word->charPos[j0 + 1];
							++j0)
						;
					for (j1 = word->len - 1;
							j1 > j0 && pos + length <= word->charPos[j1]; --j1)
						;
					switch (line->rot) {
					case 0:
						xMin1 = word->edge[j0];
...
void TextPage::dump(void *outputStream, TextOutputFunc outputFunc,
	...
	if (rawOrder) {
		...
		for (word = rawWords; word; word = word->next) {
			...
			delete s;
			if (word->next && fabs(word->next->base - word->base) <
			maxIntraLineDelta * word->fontSize
					&& word->next->xMin
							> word->xMax
									- minDupBreakOverlap * word->fontSize) {
				if (word->next->xMin
						> word->xMax + minWordSpacing * word->fontSize) {
    ...
    printf("\n");
		...
		for (i = 0; i < nFrags; ++i) {
			...
			if (i == nFrags - 1 || frags[i + 1].col < col
...
void TextPage::assignColumns(TextLineFrag *frags, int nFrags,
		bool oneRot) const {
				...
				case 0:
					if (frag0->xMin >= frag1->xMax) {
						...
						for (k = frag1->start;
								...
								++k)
							;
						col2 = frag1->col + frag1->line->col[k]
								- frag1->line->col[frag1->start];
					}
					break;
				case 1:
...
void ActualText::addChar(GfxState *state, OutputDev *dev, double x, double y,
		double dx, double dy, CharCode c, int nBytes, const Unicode *u,
		int uLen) {
	if (!actualText) {
		text->addChar(state, dev, x, y, dx, dy, c, nBytes, u, uLen);
		return;
	}
...
void ActualText::end(GfxState *state, OutputDev *dev) {
	...
	if (actualTextNBytes) {
		...
		text->addChar(state, dev, actualTextX0, actualTextY0,
				actualTextX1 - actualTextX0, actualTextY1 - actualTextY0, 0,
				actualTextNBytes, uni, length);
		gfree(uni);
	}
...
static void TextOutputDev_outputToFile(void *stream, const char *text,
		int len) {
...
void TextOutputDev::endPage() {
	...
	if (outputStream) {
		text->dump(outputStream, outputFunc, physLayout, textEOL,
				textPageBreaks);
	}
}
...
void TextOutputDev::drawChar(GfxState *state, double x, double y, double dx,
	...
	actualText->addChar(state, this, x, y, dx, dy, c, nBytes, u, uLen);
}
...
bool TextOutputDev::findText(const Unicode *s, int len, bool startAtTop,
		bool stopAtBottom, bool startAtLast, bool stopAtLast,
		bool caseSensitive, bool backward, bool wholeWord, double *xMin,
		double *yMin, double *xMax, double *yMax) const {
	return text->findText(s, len, startAtTop, stopAtBottom, startAtLast,
			stopAtLast, caseSensitive, backward, wholeWord, xMin, yMin, xMax,
			yMax);
}

GooString* TextOutputDev::getText(double xMin, double yMin, double xMax,
		double yMax) const {
	return text->getText(xMin, yMin, xMax, yMax, textEOL);
}

void TextOutputDev::drawSelection(OutputDev *out, double scale, int rotation,
		const PDFRectangle *selection, SelectionStyle style,
		const GfxColor *glyph_color, const GfxColor *box_color) {
	text->drawSelection(out, scale, rotation, selection, style, glyph_color,
			box_color);
}
...
bool TextOutputDev::findCharRange(int pos, int length, double *xMin,
		double *yMin, double *xMax, double *yMax) const {
