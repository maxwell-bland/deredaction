...
public:
  ...
  operator std::wstring() {
    ...
    if (state.get() != nullptr) {
      ...
      if (font) {
        ...
        if (fName) {
          fontInfo += (std::string(fName->c_str()) + ", " + std::to_string(state->getFontSize()));
        }
      }
    }

    /* Ugly but my compiler buffer overflows if I don't do this. */
    char strRepr[1024];
    std::wstringstream wss;
    if (rct != GfxTextRender::Shift) {
      if (rct == GfxTextRender::Space) {
        snprintf(strRepr, sizeof(strRepr), "%04X", (int) val);
        wss << strRepr;
      } else {
        wss << ((val) ? (wchar_t) val : L'-');
      }
      wss << " ";
    } else {
      snprintf(strRepr, sizeof(strRepr), "%f ", val);
      wss << strRepr;
    }
    wss << desc.c_str() << " ";
    snprintf(strRepr, sizeof(strRepr), "%.17g + ", x);
    wss << strRepr;
    if (rct == GfxTextRender::Char && sp) {
      snprintf(strRepr, sizeof(strRepr), "%.17g = ", ((unicodeIndex + 1) * dx) - sp);
      wss << strRepr;
      snprintf(strRepr, sizeof(strRepr), "%.17g y: ", (x + (unicodeIndex + 1) * dx) - sp);
    } else {
      snprintf(strRepr, sizeof(strRepr), "%.17g = ", ((unicodeIndex + 1) * dx));
      wss << strRepr;
      snprintf(strRepr, sizeof(strRepr), "%.17g y: ", (x + (unicodeIndex + 1) * dx));
    }
    wss << strRepr;
    snprintf(strRepr, sizeof(strRepr), "%.17g font: ", y);
    wss << strRepr;
    wss << fontInfo.c_str();
    ...
    /* Create a fake adjustment if there is character spacing. */
    if ((rct == GfxTextRender::Space || rct == GfxTextRender::Char) && sp) {
      GfxRenderInfo spacingShift(GfxTextRender::TJ, '0',
                                 (x + (unicodeIndex + 1) * dx) - sp,
                                 y, 0, 0, 0, sp, 0, nullptr);
      res = res + L"\n" + (std::wstring) spacingShift;
    }
  ...
  std::wstring getChar() {
    ...
    default:
      wchar_t strRepr[128];
      swprintf(strRepr, sizeof(strRepr), L"%lc",
...
public:
	...
	/**
	 * Injects a guess into the given GfxState,
   *
   * @return the GfxRenderInfo* of each character injected, in reverse order!
	 */
  std::vector<GfxRenderInfo*> injectGuess(std::wstring guess);

	/**
	 * TODO: move this back into a private context, and have injectGuess
	 * take a metadata object so that inject guess only needs to be
	 * called once, and so that this method is called just once at the
	 * beginning of injectGuess
	 */
