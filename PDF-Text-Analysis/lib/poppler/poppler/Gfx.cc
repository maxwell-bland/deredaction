		...
		if (lastS != nullptr)
			if (lastS->getCurY() == state->getCurY()) {
				out->recordRender(GfxTextRender::TJ, '0', lastS->getCurX(),
													state->getCurY(), 0, 0, 0,
													state->getCurX() - lastS->getCurX(), 0, lastS);
        GfxRenderInfo * last = out->lastRender();
        last->postState.reset( state->copy(true) );
      }
...
void Gfx::injectTextShift(int wMode, const double num) {
	...
	if (wMode) {
		...
		out->recordRender(GfxTextRender::Shift, num, state->getCurX(),
											state->getCurY(), 0, shiftAmt, 0, 0, shiftAmt, preState);
        delete preState;
...
std::vector<GfxRenderInfo *> Gfx::injectGuess(std::wstring guess) {
    /* NOTE: deleted because you shouldn't be trying to do anything 
     * with this you naughty software developer... */
